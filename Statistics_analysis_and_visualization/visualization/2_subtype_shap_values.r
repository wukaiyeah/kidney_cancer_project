...
colnames <- append(colnames, paste('svd', seq(0,63,1),sep = '_'))
colnames <- append(colnames, paste('pca',seq(0,255,1),sep = '_'))
#colnames(features_table) <- colnames

# load subtype shap values
shap_table <- read.csv('xgboost_subtype_shap_values.csv', sep = ',', header = TRUE)
...
ggplot(data = shap_mean_table, 
  ...
  coord_flip()
ggsave("subtype_cls_shap_values_mean.jpg", device = "jpeg", width = 16, height = 12, dpi = 300)
ggsave("subtype_cls_shap_values_mean.pdf", device = "pdf", width = 16, height = 12, dpi = 300)
