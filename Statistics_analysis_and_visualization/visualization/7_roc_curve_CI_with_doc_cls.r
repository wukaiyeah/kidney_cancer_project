...
# 
auc <- data.frame(x = 0.70, y = 0.062, text = c('ROC curve (Area = 0.83 +/- 0.1)'))
box <- data.frame(x = c(0.35,0.35,1,1),
                  y = c(0.01,0.12,0.12,0.01))
line_table <- data.frame(x= c(0.35, 0.40),
                         y = c(0.062,0.062))
#create ROC plot
#windowsFonts(CA=windowsFont("Calibri"))
ggplot(roc_table, aes(x = FPR, y = TPR))+
  geom_line(colour = '#61A0A8', size = 3.5)+ # roc curve
  geom_ribbon(data = ci_table, aes(x = FPR, ymin = TPR_low, ymax = TPR_high),fill='#61A0A8',alpha = .2)+
  geom_abline(slope = 1, intercept =0, linetype = 'longdash', size = 0.7, show.legend = NA)+
...
ggsave("subtype_roc_curve_ci.jpg", device = "jpeg", width = 16, height = 12, dpi = 300)
ggsave("subtype_roc_curve_ci.pdf", device = "pdf", width = 16, height = 12, dpi = 300)
...
ci_table <- ciobj[order(ciobj$FPR),]
# 
auc <- data.frame(x = 0.70, y = 0.062, text = c('ROC curve (Area = 0.80 +/- 0.1)'))
box <- data.frame(x = c(0.35,0.35,1,1),
                  y = c(0.01,0.12,0.12,0.01))
line_table <- data.frame(x= c(0.35, 0.40),
                         y = c(0.062,0.062))
...
ggsave("stage_roc_curve_ci.jpg", device = "jpeg", width = 16, height = 12, dpi = 300)
ggsave("stage_roc_curve_ci.pdf", device = "pdf", width = 16, height = 12, dpi = 300)
...
ci_table <- ciobj[order(ciobj$FPR),]
# 
auc <- data.frame(x = 0.70, y = 0.062, text = c('ROC curve (Area = 0.77 +/- 0.1)'))
box <- data.frame(x = c(0.35,0.35,1,1),
                  y = c(0.01,0.12,0.12,0.01))
line_table <- data.frame(x= c(0.35, 0.40),
                         y = c(0.062,0.062))
...
ggsave("grade_roc_curve_ci.jpg", device = "jpeg", width = 16, height = 12, dpi = 300)
ggsave("grade_roc_curve_ci.pdf", device = "pdf", width = 16, height = 12, dpi = 300)
