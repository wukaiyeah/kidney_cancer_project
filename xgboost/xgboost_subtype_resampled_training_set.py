...
if __name__ == '__main__':
    ...
    model = XGBClassifier(scale_pos_weight = 2, # the best 2
                        min_child_weight = 0.6,# the best0.6
                        #learning_rate = 0.05,
                        n_estimators = 300,
                        num_boost_round = 300,
                        gamma = 0.05,# the best 0.05
                        #max_depth = 10,
                        reg_lambda = 30,# the best 30
                        subsample = 0.7, # the best 0.7
                        colsample_bytree = 1 # the best 1
                            )
    ...
    test_pred_table = pd.concat((pd.DataFrame(np.array(test_y)), pd.DataFrame(test_y_pred_proba)), axis=1)
    test_pred_table.columns = ['truth','proba']
    test_pred_table.to_csv('subtype_predict_proba_resampled_train.csv', index = 0)
    # McNemer's test
    y_target = test_y
