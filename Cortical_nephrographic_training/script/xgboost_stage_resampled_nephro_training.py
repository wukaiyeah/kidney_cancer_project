...
if __name__ == '__main__':
    ...
    train_feature = pd.read_csv(os.path.join(base_dir, 'train_feat_nephro.csv'), header = 0, index_col=0)
    #test_feature1 = pd.read_csv(os.path.join(base_dir, 'test_feat_cort.csv'), header = 0, index_col=0)
    test_feature1 = pd.read_csv(os.path.join(base_dir, 'test_feat_nephro_pred_nephro.csv'), header = 0, index_col=0)
    test_feature2 = pd.read_csv(os.path.join(base_dir, 'test_feat_nephro_pred_cort.csv'), header = 0, index_col=0)
    ...
    model = XGBClassifier(scale_pos_weight = 1.4, # the best 1.4
                        min_child_weight = 1,
                        #learning_rate = 0.01,
                        n_estimators=300,
                        num_boost_round = 300,
                        gamma = 0.15,
                        #max_depth = 10,
                        reg_lambda = 1.6, # the best 1.1
                        subsample = 1, # the best 0.8
                        colsample_bytree = 1) # the best)

    eval_set = [(test_x, test_y)]
    model.fit(train_x,train_y, early_stopping_rounds=200, eval_metric=['auc','error','logloss'], eval_set=eval_set, verbose=True)
    #'logloss',,'error'
    ...
    test_pred_table.to_csv('stage_proba_nephro_pred_cort.csv', index=0)
