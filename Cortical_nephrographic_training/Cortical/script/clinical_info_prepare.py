...
if __name__ == '__main__':
    base_dir = '/mnt/d/AI_diagnosis/Renal_cancer_project/Cortical_nephrographic_training/Cortical'
    
    total_case_info = pd.read_csv(os.path.join(base_dir, 'clinical_all_rcc_dict.csv'), header=0)
    train_id = pd.read_table(os.path.join(base_dir, 'phase2_train_id.txt'), header=None)
    test_id = pd.read_table(os.path.join(base_dir, 'phase2_test_id.txt'), header=None)
    ...
    clin_info.to_csv(os.path.join(base_dir, 'cort_classes_label.csv'),index=None)
    print('ok')
